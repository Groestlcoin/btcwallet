...
func TestOutputSplittingNotEnoughInputs(t *testing.T) {
	...
	requests := []OutputRequest{
		...
		TstNewOutputRequest(t, 1, "34eVkREKgvvGASZW7hkgE2uNc1yya1UDSZ", output1Amount, net),
		TstNewOutputRequest(t, 2, "34eVkREKgvvGASZW7hkgE2uNc1yya1UDSZ", output2Amount, net),
	}
...
func TestOutputSplittingOversizeTx(t *testing.T) {
	...
	request := TstNewOutputRequest(
		t, 1, "34eVkREKgvvGASZW7hkgE2uNc1yya1UDSZ", requestAmount, pool.Manager().ChainParams())
...
func TestWithdrawalTxOutputs(t *testing.T) {
	...
	outputs := []OutputRequest{
		TstNewOutputRequest(t, 1, "34eVkREKgvvGASZW7hkgE2uNc1yya1UDSZ", 3e6, net),
		TstNewOutputRequest(t, 2, "3PbExiaztsSYgh6zeMswC49hLUwhUn71xC", 2e6, net),
	}
	changeStart := TstNewChangeAddress(t, pool, seriesID, 0)
...
func TestFulfillRequestsNoSatisfiableOutputs(t *testing.T) {
	...
	request := TstNewOutputRequest(
		t, 1, "3Qt1EaKRD9g9FeL2DGkLLswhK1AKkubMkH", btcutil.Amount(3e6), pool.Manager().ChainParams())
	changeStart := TstNewChangeAddress(t, pool, seriesID, 0)
...
func TestFulfillRequestsNotEnoughCreditsForAllRequests(t *testing.T) {
	...
	out1 := TstNewOutputRequest(
		t, 1, "34eVkREKgvvGASZW7hkgE2uNc1yya1UDSZ", btcutil.Amount(3e6), net)
	out2 := TstNewOutputRequest(
		t, 2, "3PbExiaztsSYgh6zeMswC49hLUwhUn71xC", btcutil.Amount(2e6), net)
	out3 := TstNewOutputRequest(
		t, 3, "3Qt1EaKRD9g9FeL2DGkLLswhK1AKkubMkH", btcutil.Amount(5e6), net)
...
func TestRollBackLastOutputInsufficientOutputs(t *testing.T) {
	...
	output := &WithdrawalOutput{request: TstNewOutputRequest(
		t, 1, "34eVkREKgvvGASZW7hkgE2uNc1yya1UDSZ", btcutil.Amount(3), &chaincfg.MainNetParams)}
...
func TestRollbackLastOutputWhenNewOutputAdded(t *testing.T) {
	...
	requests := []OutputRequest{
		...
		TstNewOutputRequest(t, 1, "34eVkREKgvvGASZW7hkgE2uNc1yya1UDSZ", 1, net),
		TstNewOutputRequest(t, 2, "3PbExiaztsSYgh6zeMswC49hLUwhUn71xC", 2, net),
	}
...
func TestRollbackLastOutputWhenNewInputAdded(t *testing.T) {
	...
	requests := []OutputRequest{
		...
		TstNewOutputRequest(t, 1, "34eVkREKgvvGASZW7hkgE2uNc1yya1UDSZ", 1, net),
		TstNewOutputRequest(t, 3, "3Qt1EaKRD9g9FeL2DGkLLswhK1AKkubMkH", 6, net),
		TstNewOutputRequest(t, 2, "3PbExiaztsSYgh6zeMswC49hLUwhUn71xC", 3, net),
	}
	changeStart := TstNewChangeAddress(t, pool, series, 0)
...
func TestSignMultiSigUTXORedeemScriptNotFound(t *testing.T) {
	...
	// script.
	addr, _ := btcutil.DecodeAddress("3Hb4xcebcKg4DiETJfwjh8sF4uDw6EnbtN", mgr.ChainParams())
